...
class Genetic extends Serializable {
  ...
  def CrossMultipoint(Variables: TableVar, clas: Int, dad: Int, mom: Int, contador: Int, neje: Int) {
    ...
    if (cruce <= getProbCross) {
      ...
      offspring(clas).setIndivEvaluated(contador * 2, false)
      offspring(clas).indivi(contador * 2).cubre.clear(0, offspring(clas).indivi(contador * 2).cubre.size())
      offspring(clas).setIndivEvaluated((contador * 2) + 1, false)
      offspring(clas).indivi((contador * 2) + 1).cubre.clear(0, offspring(clas).indivi((contador * 2) + 1).cubre.size())
    } /*else {
    ...
    if (getProbMutation > 0) {
      for (i <- 0 until posiciones) {
        ...
        if (mutar <= getProbMutation) {
          ...
          offspring(clas).indivi(pos).cubre.clear(0, offspring(clas).indivi(pos).cubre.size())
        }
  ...
  def GeneticAlgorithm(Variables: Broadcast[TableVar], Examples: TableDat, nFile: String, sc: SparkContext): Population = {
    ...
    poblac.foreach(p => {
      p.indivi.foreach(ind => {
        auxiliar.CopyIndiv(count, Examples.getNEx, getNumObjectives, ind)
        count += 1
      })
    })

    val popToRet = auxiliar.tokenCompetition(Examples, Variables.value, this)
  ...
  */
  ...
  def evaluatePopulation(poblac: Array[Population], Variables: Broadcast[TableVar], Examples: TableDat, sc: SparkContext, fin: Boolean): Int = {
    ...
    auxPob.foreach(y => {
      y.foreach(x => {
        x.setIndivEvaluated(true)
        x.setNEval(getTrials + ninds - k)
        k = k - 1
      })
    })
