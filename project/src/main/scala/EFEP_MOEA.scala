...
object EFEP_MOEA {
  ...
  def CalculateTest(outputFile: String, measureFile:String,  pob: Population, classificationType: String, ruleRep: String, sc: SparkContext, AG: Genetic) {
    ...
    contents = "Number \tClass \tSize \tNVar \tTP \tFP \tTN \tFN \tLength \tUnusualness \tGain" + "\tSensitivity \tSupportP \tSupporN \tSupportDif \tFConfidence \tGrowthRate \tTEFisher \tHellinger \tTPr \tFPr \n"
    ...
    pob.indivi.foreach(ind => {
      ...
      contents += "\t" + sixDecimals.format(measures.confusionMatrix.tp)
      contents += "\t" + sixDecimals.format(measures.confusionMatrix.fp)
      contents += "\t" + sixDecimals.format(measures.confusionMatrix.tn)
      contents += "\t" + sixDecimals.format(measures.confusionMatrix.fn)
